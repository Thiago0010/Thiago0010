# Thiago Caselato de Mendonça
**Software Engineer | Founder @ [NEV]²**

> Construindo sistemas resilientes e soluções de software que transcendem a teoria para resolver problemas do mundo real.

---

### ⚡ Sobre mim

Sou um desenvolvedor movido pela curiosidade técnica e pelo rigor da engenharia. Meu foco está na criação de soluções completas, desde a arquitetura de sistemas de baixo nível até aplicações web e móveis escaláveis. 

Como fundador e integrante do time **[NEV]²**, lidero e participo de projetos que prezam pela autonomia técnica e pela construção de ferramentas autorais. Acredito que a tecnologia deve ser compreendida em sua essência, o que me leva a explorar constantemente a integração entre software e hardware.

---

### 🎯 Áreas de Atuação

* **Sistemas:** Desenvolvimento de arquiteturas robustas e lógica de backend.
* **Desenvolvimento Web:** Aplicações dinâmicas, performáticas e responsivas.
* **Aplicativos:** Soluções mobile focadas em experiência do usuário e eficiência.
* **Automações:** Otimização de processos e workflows inteligentes.
* **IoT / ESP32:** Integração de hardware e software para controle de ambientes e dispositivos.
* **Projetos Experimentais:** Exploração de linguagens de baixo nível e pesquisa técnica.

---

### 🛠 Tecnologias e Ferramentas

**Linguagens de Programação**
![C](https://img.shields.io/badge/c-%2300599C.svg?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=flat-square&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=flat-square&logo=javascript&logoColor=%23F7DF1E)
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=flat-square&logo=typescript&logoColor=white)
![Rust](https://img.shields.io/badge/rust-%23000000.svg?style=flat-square&logo=rust&logoColor=white)

**Frameworks e Ecossistema**
![React](https://img.shields.io/badge/react-%2320232a.svg?style=flat-square&logo=react&logoColor=%2361DAFB)
![NodeJS](https://img.shields.io/badge/node.js-6DA55F?style=flat-square&logo=node.js&logoColor=white)
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=flat-square&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=flat-square&logo=git&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/postgres-%23316192.svg?style=flat-square&logo=postgresql&logoColor=white)

**Hardware e Microcontroladores**
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=flat-square&logo=espressif&logoColor=white)
![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=flat-square&logo=Arduino&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/-Raspberry%20Pi-C51A4A?style=flat-square&logo=Raspberry-Pi)

---

### 🚀 Projetos e Iniciativas

#### **[NEV]²**
Projetos desenvolvidos em conjunto com o time, focados em inovação e ferramentas de código aberto.
* **MyFreela:** Dashboard Profiissional Para gerenciamento de freelances.
* **Parreias Do Sul:** Landing Page Para uma marca de sucos do Sul chamado Parreiras Do Sul.

#### **Destaques Pessoais**
* **NevEdu:** Sistema para escolas integrando IA para criação de cursos especializados.
* **[NEV]²OS:** Sistema Operacional em 64 BITS.

---

### 🔭 Visão

> "A verdadeira soberania digital nasce da capacidade de construir seus próprios sistemas, do bit ao pixel."

---


### SITES
https://thiago0010.github.io/-NEV-Thi-DEV/
https://nev-bot-automations.infinityfree.me/?i=1
https://caldasdiromalocacoes.kesug.com/
https://parreirasdosul.com/
https://www.slimlifereviews.com
https://jjpereiraprodutosnaturais.netlify.app/

### 📬 Contato

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/thiago0010)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/nev.thi.ii)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:caselasthiago@gmail.com)

---
